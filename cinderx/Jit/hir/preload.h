...
class Preloader {
 public:
  ...
  //
  // Preloaders are keyed by code object and shared across closures, so this
  // snapshot may not match every closure. If it differs, the inliner's
  // func_defaults guard deopts.
 ...
 private:
  ...
  Ref<PyTupleObject> func_defaults_;
